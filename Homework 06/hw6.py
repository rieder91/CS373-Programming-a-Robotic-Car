...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        # n is the robot position in the matrix
        n = 0
        Omega = Omega.expand(dim + 2, dim + 2, list1, list2)
        ...
        for i in range(len(measurement)):
            ...
            # m is the landmark index
            m = 2 * (2 + measurement[i][0])
    
            # matrix/vector update
            for j in range(2):
                Omega.value[n + j][n + j] +=  1. / measurement_noise
                Omega.value[m + j][m + j] +=  1. / measurement_noise
                Omega.value[n + j][m + j] += -1. / measurement_noise
                Omega.value[m + j][n + j] += -1. / measurement_noise
                Xi.value[n + j][0] += -measurement[i][1+j] / measurement_noise
                Xi.value[m + j][0] +=  measurement[i][1+j] / measurement_noise


        # motion update
        for i in range(4):
            Omega.value[n + i][n + i] +=  1. / motion_noise
        for i in range(2):
            Omega.value[n + i][n + i + 2] += -1. / motion_noise
            Omega.value[n + i + 2][n + i] += -1. / motion_noise
            Xi.value[n + i][0] += -motion[i] / motion_noise
            Xi.value[n + i + 2][0] +=  motion[i] / motion_noise
        
    
        # get temporary matrices
        A = Omega.take([0, 1], list3)
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        Omega1 = Omega.take(list3, list3)
        Xi1 = Xi.take(list3, [0])

        # compute new matrices for next round
...
print 'Running full slam routine'
...
print 'Running online slam routine'
data = make_data(N, num_landmarks, world_size, measurement_range, motion_noise, measurement_noise, distance)
